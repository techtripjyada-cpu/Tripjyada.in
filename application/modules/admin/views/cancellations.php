...
<title>Cancellations — Tripjyada Admin</title>
...
<div class="topbar">
    <div style="display:flex;align-items:center;gap:20px">
        ...
        <nav class="topbar-nav">
            ...
            <a href="<?= site_url('admin/cancellations') ?>" class="active">Cancellations <?php if(!empty($admin_stats['cancel_pending'])): ?><span style="background:#dc2626;color:#fff;font-size:10px;padding:1px 6px;border-radius:20px;margin-left:3px"><?= $admin_stats['cancel_pending'] ?></span><?php endif; ?></a>
...
<div class="main">
    <div class="toolbar">
        <a href="?status=" class="filter-btn <?= $status === '' ? 'active' : '' ?>">All (<?= $admin_stats['cancel_pending'] + 0 ?>)</a>
        <a href="?status=pending" class="filter-btn <?= $status === 'pending' ? 'active' : '' ?>">Pending <?php if($admin_stats['cancel_pending']): ?><span style="background:#dc2626;color:#fff;font-size:10px;padding:1px 6px;border-radius:20px;margin-left:3px"><?= $admin_stats['cancel_pending'] ?></span><?php endif; ?></a>
    ...
    <div class="table-wrap">
        <table>
            <thead><tr><th>#</th><th>User</th><th>Booking Ref</th><th>Reason</th><th>Requested</th><th>Status</th><th>Admin Note</th><th>Action</th></tr></thead>
            <tbody>
            <?php if (empty($requests)): ?>
            <tr><td colspan="8" class="empty">No cancellation requests found.</td></tr>
            ...
            <tr id="row-<?= $r['id'] ?>">
                ...
                <td style="max-width:200px">
                    <div style="font-size:12px;color:#475569;line-height:1.4"><?= htmlspecialchars(substr($r['reason'], 0, 120)) ?><?= strlen($r['reason']) > 120 ? '…' : '' ?></div>
                </td>
                <td style="font-size:12px;color:#64748b"><?= date('d M Y', strtotime($r['created_at'])) ?></td>
                <td><span class="badge badge-<?= $r['status'] ?>"><?= $r['status'] ?></span></td>
                <td>
                    <div style="font-size:11px;color:#64748b;max-width:150px"><?= htmlspecialchars($r['admin_note'] ?: '—') ?></div>
                </td>
                <td>
                    ...
                    <div style="display:flex;flex-direction:column;gap:6px">
                        ...
                        <div style="display:flex;gap:5px">
                            <button class="action-btn btn-approve" onclick="updateCancel(<?= $r['id'] ?>,'approved')">Approve</button>
                            <button class="action-btn btn-reject" onclick="updateCancel(<?= $r['id'] ?>,'rejected')">Reject</button>
                        </div>
                    </div>
                    <?php else: ?>
...
function updateCancel(id, status) {
    ...
    fetch('<?= site_url('admin/update-cancellation') ?>/' + id, {method:'POST', body:fd})
        ...
        .then(function(d){
            ...
            if (d.ok) {
                ...
                if (row) {
                    ...
                    if (noteCell && noteVal) noteCell.innerHTML = '<div style="font-size:11px;color:#64748b;max-width:150px">' + noteVal + '</div>';
                }
            }
        })
