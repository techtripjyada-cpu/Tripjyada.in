...
$category_name  = isset($category_names[$package['category']]) ? $category_names[$package['category']] : ucwords(str_replace('-', ' ', $package['category']));
$category_slug  = isset($category_slug_map[$package['category']]) ? $category_slug_map[$package['category']] : 'group-tour';
$details_text   = !empty($package['details'])   ? $package['details']   : '';
$details_has_html = $details_text !== strip_tags($details_text);
...
<nav class="pd-breadcrumb-bar" aria-label="Breadcrumb">
  <div class="container">
    <ol class="pd-breadcrumb-list">
      ...
      <li><span class="pd-sep">›</span><a href="<?= site_url('tour-package/' . $category_slug) ?>"><?= htmlspecialchars($category_name) ?> Packages</a></li>
    ...
    <div class="row">
      ...
      <div class="col-lg-8 col-12">
        ...
        <div class="mb-4">
          ...
          <?php if ($details_has_html): ?>
            <div class="pd-richtext"><?= $details_text ?></div>
          <?php else: ?>
            <?php foreach (explode("\n\n", trim($details_text)) as $para): ?>
              ...
              <div class="pd-day-body" <?= $i === 0 ? 'style="display:block"' : '' ?>>
                ...
                <div class="pd-day-meta">
                  ...
                  <?php if (!empty($day['meals'])): ?>
                    <span class="pd-day-tag pd-day-tag-meals">🍽️ Meals: <?= htmlspecialchars($day['meals']) ?></span>
                  <?php endif; ?>
                </div>
              </div>
            </div>
      ...
      <div class="col-lg-4 col-12 mt-4 mt-lg-0">
        ...
        <div class="pd-sidebar-card">
          ...
          <ul class="pd-facts">
            ...
            <li><span class="fact-label">Language</span><span class="fact-value">English Guide</span></li>
            <li><span class="fact-label">Price</span><span class="fact-value" style="color:#f97316">
              <?php if (!empty($package['price_on_request'])): ?>Contact for Price<?php elseif ($package_price_rupees > 0): ?>&#8377;<?= number_format($package_price_rupees) ?><?php else: ?>&#8377;<?= htmlspecialchars($package['price']) ?><?php endif; ?>
            </span></li>
          </ul>
        </div>
        </div><!-- /sticky -->
...
function pdFaqToggle(qEl) {
  var item = qEl.parentElement;
  var answer = item.querySelector('.pd-faq-a');
  var isOpen = item.classList.contains('open');
  document.querySelectorAll('.pd-faq-item').forEach(function(f) {
    f.classList.remove('open');
    f.querySelector('.pd-faq-a').style.display = 'none';
  });
